...
public:
	...
	void OnRun()
	{
		...
		while (isRun())
		{
			...
			/*
			备份FD_READ集合的目的：
			1.正是客户端队列无变化时不需要重新计算maxSock
			2.不用每次都循环将_client里的所有sock取出放入fd_read，而可以直接赋值
			*/
	...
	//接收数据 处理粘包 拆分包   //接收客户端数据在主线程进行，直接写入每个客户端类的接收缓冲区
	int RecvData(CellClient* pClient)
	{
		...
		//如果客户端接收缓冲区数据大于整个消息消息的长度，则进行消息处理并把接收缓冲区的数据进行偏移
	...
	void Start()
	{
		...
		_taskServer.Start();  //新开一个线程执行发送任务服务
	}
	...
	//在子服务器线程内添加发送任务到发送任务服务器，包含要发送目标以及网络消息
...
private:
	...
	//在子服务器线程内添加发送任务到发送任务服务器
	CellTaskServer _taskServer;  //每个子服务器又包含一个发送任务服务器
};
